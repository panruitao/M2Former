...
class RTDETRDetectionModel(DetectionModel):
    ...
    def init_criterion(self):
        ...
        if self.args.use_mal:
            return RTDETRDetectionLoss(nc=self.nc,
                                       imgsz=self.args.imgsz,
                                       use_mal=True,
                                       label_smoothing=self.args.class_smoothing,
                                       use_aal=self.args.use_aal,
                                       use_topk=self.args.use_topk)
        else:
            return RTDETRDetectionLoss(nc=self.nc,
                                       imgsz=self.args.imgsz,
                                       use_vfl=True,
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    base_modules = frozenset(
        {
            ...
            C2fCIB,
            A2C2f,
            ConvNormLayer,  # AddModules: ResNet
            Blocks,  # AddModules: ResNet
            M2FormerBlocks,  # AddModules: M2Former
        }
    )
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        m_.i, m_.f, m_.type = i + 4 if backbone else i, f, t
        if verbose:
            LOGGER.info(f"{i:>3}{str(f):>20}{n_:>3}{m_.np:10.0f}  {t:<45}{str(args):<30}")  # print
        # save.extend(x % i for x in ([f] if isinstance(f, int) else f) if x != -1)  # append to savelist
        save.extend(x % (i + 4 if backbone else i) for x in ([f] if isinstance(f, int) else f) if x != -1)
        layers.append(m_)
        if i == 0:
            ch = []
        if isinstance(c2, list):  # AddModules: PoolFormer
            ch.extend(c2)
            for _ in range(5 - len(c2)):
                ch.insert(0, 0)
        else:
            ch.append(c2)
    return torch.nn.Sequential(*layers), sorted(list(set(save)))
