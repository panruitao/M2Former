...
class Mosaic(BaseMixTransform):
    ...
    def _mosaic4(self, labels):
        ...
        if hasattr(self.dataset, "representation"):
            fill_value = 0
        else:
            fill_value = 114
...
class RandomPerspective:
    ...
    def __init__(
        self, dataset, degrees=0.0, translate=0.1, scale=0.5, shear=0.0, perspective=0.0, border=(0, 0), pre_transform=None
    ):
        """
    ...
    def affine_transform(self, img, border):
        ...
        if (border[0] != 0) or (border[1] != 0) or (M != np.eye(3)).any():  # image changed
            if self.perspective:
                img = cv2.warpPerspective(img, M, dsize=self.size, borderValue=borderValue)
            else:  # affine
                img = cv2.warpAffine(img, M[:2], dsize=self.size, borderValue=borderValue)
        return img, M, s
...
class LetterBox_event:
    ...
    def __call__(self, labels=None, image=None):
        ...
        if self.auto:  # minimum rectangle
            ...
            dw, dh = 0.0, 0.0
            new_unpad = (new_shape[0], new_shape[1])  # [height, width]
            ratio = new_shape[1] / shape[1], new_shape[0] / shape[0]  # width, height ratios
        ...
        if shape != new_unpad:  # resize
            img = resize_event(img, new_unpad)
        top, bottom = int(round(dh - 0.1)) if self.center else 0, int(round(dh + 0.1))
        left, right = int(round(dw - 0.1)) if self.center else 0, int(round(dw + 0.1))
        img = np.pad(img, ((top, bottom), (left, right), (0, 0)), mode='constant', constant_values=0)  # add border
...
class CopyPaste(BaseMixTransform):
    ...
    def __call__(self, labels):
        ...
        if len(labels["instances"].segments) == 0 or self.p == 0:
            if (hasattr(self.dataset, "representation") and
                len(labels["instances"].bboxes) != 0):
                pass
            else:
                return labels
    ...
    def _transform(self, labels1, labels2={}):
        ...
        if hasattr(self.dataset, "representation"):
            mask = instances2.bbox_areas < 900
            if not mask.any():
                return labels1
            instances2 = instances2[mask]
            cls2 = labels2.get("cls", cls)[mask]
        ...
        if hasattr(self.dataset, "representation"):
            for j in indexes[: round(self.p * n)]:
                ...
                x1, y1, x2, y2 = instances2.bboxes[j].astype(int)
                channels = im_new.shape[2]
                color = (1,) * channels
                cv2.rectangle(im_new, (x1, y1), (x2, y2), color, thickness=cv2.FILLED)
        else:
            for j in indexes[: round(self.p * n)]:
...
def v8_transforms_event(dataset, imgsz, hyp, stretch=False):
    mosaic = Mosaic(dataset, imgsz=imgsz, p=hyp.mosaic)
    affine = RandomPerspective(
        dataset,
        degrees=hyp.degrees,
        ...
        shear=hyp.shear,
        perspective=hyp.perspective,
        pre_transform=None if stretch else LetterBox_event(new_shape=(imgsz, imgsz)),
    )
...
def resize_event(event, shape):
    h0, w0, ch = event.shape
    h1, w1 = shape[0], shape[1]
    r_h, r_w = h1 / h0, w1 / w0

    output = np.zeros((h1, w1, ch), dtype=event.dtype)
    for i in range(ch):
        sparse_mat = coo_matrix(event[:, :, i])
        y_coords, x_coords, values = sparse_mat.row, sparse_mat.col, sparse_mat.data
        new_y_coords = np.clip(np.round(y_coords * r_h).astype(int), 0, h1 - 1)
        new_x_coords = np.clip(np.round(x_coords * r_w).astype(int), 0, w1 - 1)
        np.add.at(output[:, :, i], (new_y_coords, new_x_coords), values)
