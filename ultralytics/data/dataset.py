...
class EventDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        h5_file = os.path.join(img_path, f'{self.representation}.h5')
        if os.path.exists(h5_file):
            self.h5_file = h5_file
        else:
            raise FileNotFoundError(f'{prefix}{h5_file} dose not exist')

        self.single_cls = single_cls
        self.prefix = prefix
        self.fraction = fraction
        self.im_files = self.get_img_files_h5(self.h5_file)
    ...
    def get_img_files(self, img_path):
        ...
        try:
            ...
            im_files = sorted(x.replace("/", os.sep) for x in f if x.split(".")[-1].lower() == 'txt')
    ...
    def get_img_files_h5(self, h5_file):
        """Read image keys from h5 file."""
        try:
            p = Path(h5_file)
            assert p.suffix == '.h5' and p.is_file(), f"{self.prefix}{p} is not a valid .h5 file."

            with h5py.File(p, 'r') as f:
                keys = list(f.keys())
                folder = Path(self.img_path)
                im_files = [str(folder / k) for k in keys]
            assert im_files, f"{self.prefix}No image keys found in {h5_file}."
        except Exception as e:
            raise FileNotFoundError(f"{self.prefix}Error loading keys from {h5_file}\n{HELP_URL}") from e
    ...
    def get_labels(self):
        """Returns dictionary of labels for YOLO training."""
        self.label_files = event2label_paths(self.im_files)
        cache_path = Path(self.label_files[0]).parent.with_suffix(".cache")
        try:
            cache, exists = load_dataset_cache_file(cache_path), True  # attempt to load a *.cache file
            assert cache["version"] == DATASET_CACHE_VERSION  # matches current version
    ...
    def check_cache_ram(self, safety_margin=0.5):
        ...
        for _ in range(n):
            im = self.read_event(random.choice(self.im_files))  # sample image
            if im is None:
                continue
    ...
    def cache_images_to_disk(self, i):
        ...
        f = self.npy_files[i]
        if not f.exists():
            np.save(f.as_posix(), self.read_event(self.im_files[i]), allow_pickle=False)
    ...
    def build_transforms(self, hyp=None):
        ...
        if self.augment:
            hyp.mosaic = hyp.mosaic if self.augment and not self.rect else 0.0
            hyp.mixup = hyp.mixup if self.augment and not self.rect else 0.0
            transforms = v8_transforms_event(self, self.imgsz, hyp)
        else:
            transforms = Compose([LetterBox_event(new_shape=(self.imgsz, self.imgsz), scaleup=False)])
        transforms.append(
            Format_event(
                bbox_format="xywh",
                normalize=True,
    ...
    def load_image(self, i, rect_mode=True):
        ...
        if rect_mode:  # resize long side to imgsz while maintaining aspect ratio
            ...
            if r != 1:  # if sizes are not equal
                h, w = (min(math.ceil(h0 * r), self.imgsz), min(math.ceil(w0 * r), self.imgsz))
                im = resize_event(im, (h, w))
        elif not (h0 == w0 == self.imgsz):  # resize by stretching image to square imgsz
            im = resize_event(im, (self.imgsz, self.imgsz))
