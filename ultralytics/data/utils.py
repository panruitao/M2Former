...
def event2label_paths(event_paths):
    sa, sb = f'{os.sep}hdf5{os.sep}', f'{os.sep}labels{os.sep}'  # /hdf5/, /labels/ substrings
    return [sb.join(x.rsplit(sa, 1)).rsplit('.', 1)[0] + '.txt' for x in event_paths]
...
def verify_event_label(args):
    im_file, lb_file, h5_file, prefix, keypoint, num_cls, nkpt, ndim = args
    # Number (missing, found, empty, corrupt), message, segments, keypoints
    nm, nf, ne, nc, msg, segments, keypoints = 0, 0, 0, 0, "", [], None
    try:
        # verify events
        ...
        with h5py.File(h5_file, "r") as f:
            if name in f:
                ...
                raise FileNotFoundError(f'{prefix}Event {name} not found in {h5_file}')
        shape = event_data.shape[:2]
        assert (shape[0] > 0) & (shape[1] > 0), f'event shape {shape} invalid (must be >0)'
