...
def adaptive_wasserstein_loss(pred, target, area, eps=1e-7, constant=12.8):
    '''
    MatchAware loss by Zheng et al.

    https://arxiv.org/abs/2005.03572.
    '''

    px, py, pw, ph = pred.chunk(4, dim=-1)
    tx, ty, tw, th = target.chunk(4, dim=-1)

    center_distance = (px - tx) ** 2 + (py - ty) ** 2 + eps
    wh_distance = ((pw - tw) ** 2 + (ph - th) ** 2) / 4 + eps
    wasserstein_2 = center_distance + wh_distance  # (n, 1)

    # Compute adaptive constant based on bbox area
    ratio = 1 / (torch.sigmoid(torch.log(1.0 / area)))
    adaptive_constant = (ratio * constant / 2).unsqueeze(-1)  # (n, 1)

    return torch.exp(-torch.sqrt(wasserstein_2) / adaptive_constant)


# def adaptive_l1_loss(pred, target):
#     area = (target[:, 2] * target[:, 3]).clamp(min=1e-6)
#     weight = 2 * torch.sigmoid(torch.log(1.0 / area + 1.0)) - 1
#     l1 = F.l1_loss(pred, target, reduction='none').sum(dim=1)
#     return (l1 * weight).sum()
