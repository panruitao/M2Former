...
class VarifocalLossWithSmoothing(nn.Module):
    """
    Varifocal loss with optional label smoothing.
    """
    ...
    def __init__(self, alpha=0.75, gamma=2.0, smoothing=0.0):
        ...
        Args:
            ...
            smoothing (float): label smoothing factor, typically 0.0 ~ 0.2
        """
        super().__init__()
        self.alpha = alpha
        self.gamma = gamma
    ...
    def forward(self, pred_score, gt_score, label):
        if self.smoothing > 0:
            label = label * (1.0 - self.smoothing) + self.smoothing
...
class MatchAwareLoss(nn.Module):
    """
    MatchAware loss by Huang et al.
    ...
    @staticmethod
    def forward(pred_score, gt_score, label, gamma=1.5):
...
class MatchAwareLossWithSmoothing(nn.Module):
    """
    Match-Aware Loss with optional label smoothing.
    """

    def __init__(self, gamma=1.5, smoothing=0.0):
        """
        Args:
            gamma (float): focusing parameter.
            smoothing (float): label smoothing factor, typically 0.0 ~ 0.2
    ...
    def forward(self, pred_score, gt_score, label):
        if self.smoothing > 0:
            label = label * (1.0 - self.smoothing) + self.smoothing
        gt_score = gt_score.pow(self.gamma)
        weight = pred_score.sigmoid().pow(self.gamma) * (1 - label) + label
        with autocast(enabled=False):
            loss = (
                (F.binary_cross_entropy_with_logits(pred_score.float(), gt_score.float(), reduction="none") * weight)
                .mean(1)
