...
def plot_events(
    ...
    processed_events = []
    for i in range(bs):
        event = images[i]  # (C, H, W)
        if c > 1:  # volume representation
            n_bins = c // 2
            pos_event = np.sum(event[n_bins:], axis=0)
            neg_event = np.sum(event[:n_bins], axis=0)
            if np.max(pos_event) > 0:
                pos_event /= np.max(pos_event)
            if np.max(neg_event) > 0:
                neg_event /= np.max(neg_event)
        else:  # frame/surface representation
            pos_event = np.clip(event[0], 0, 1)
            neg_event = np.clip(-event[0], 0, 1)

        pos_event = (pos_event * 255).astype(np.uint8)
        neg_event = (neg_event * 255).astype(np.uint8)

        event_rgb = np.zeros((h, w, 3), dtype=np.uint8)
        event_rgb[:, :, 0] = pos_event  # red color
        event_rgb[:, :, 2] = neg_event  # blue color
        event_rgb[:, :, 0] = np.clip(event_rgb[:, :, 0] * 2, 0, 255)
        event_rgb[:, :, 2] = np.clip(event_rgb[:, :, 2] * 2, 0, 255)

        threshold = 20
        mask = (event_rgb[:, :, 0] > threshold) | (event_rgb[:, :, 2] > threshold)
        event_rgb[mask] = np.clip(event_rgb[mask] + 80, 0, 255)
        processed_events.append(event_rgb)

    images = np.array(processed_events)  # (B, H, W, 3)

    # Build Image
    mosaic = np.full((int(ns * h), int(ns * w), 3), 255, dtype=np.uint8)  # init
    for i in range(bs):
        x, y = int(w * (i // ns)), int(h * (i % ns))  # block origin
        mosaic[y : y + h, x : x + w, :] = images[i]
