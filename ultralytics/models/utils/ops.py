...
class HungarianMatcher(nn.Module):
    ...
    def __init__(self, cost_gain=None, use_fl=True, with_mask=False,
                 num_sample_points=12544, alpha=0.25, gamma=2.0, topk=0):
        """
    ...
    # This forward function is for Top-k matching
    # def forward(self, pred_bboxes, pred_scores, gt_bboxes, gt_cls, gt_groups, masks=None, gt_mask=None):
    #     bs, nq, nc = pred_scores.shape
    #
    #     if sum(gt_groups) == 0:
    ...
    #     C = C.view(bs, nq, -1).cpu()
    #     indices = [linear_sum_assignment(c[i]) for i, c in enumerate(C.split(gt_groups, -1))]
    #     gt_groups_batch = torch.as_tensor([0, *gt_groups[:-1]]).cumsum_(0)  # (idx for queries, idx for gt)
    #
    #     # return [
    #     #     (torch.tensor(i, dtype=torch.long), torch.tensor(j, dtype=torch.long) + gt_groups[k])
    #     #     for k, (i, j) in enumerate(indices)
    #     # ]
    #
    #     # Standard Hungarian matching output
    #     hungarian_match_indices = [
    #         (torch.as_tensor(i, dtype=torch.long), torch.as_tensor(j, dtype=torch.long) + gt_groups_batch[k])
    #         for k, (i, j) in enumerate(indices)
    #     ]
    #
    #     # === Top-K Matching using Cost Matrix ===
    #     topk_match_indices = []
    #     if self.topk > 0:
    #         for b in range(bs):
    #             c = C[b].clone()  # [nq, num_gt]
    #             num_gt = gt_groups[b]
    #             if num_gt == 0:
    #                 topk_match_indices.append((
    #                     torch.zeros(0, dtype=torch.long),
    #                     torch.zeros(0, dtype=torch.long)
    #                 ))
    #                 continue
    #
    #             # Step 1: Mask out Hungarian matches
    #             hungarian_q, hungarian_g = indices[b]
    #             if len(hungarian_q) > 0:
    #                 c[hungarian_q, hungarian_g] = float("inf")  # mask out
    #
    #             # Step 2: Collect candidate (q, g, cost) per GT
    #             all_candidates = []
    #             for gt_i in range(num_gt):
    #                 col = c[:, gt_i]  # [nq]
    #                 sorted_costs, sorted_idx = torch.sort(col)  # ascending
    #                 for i in range(min(self.topk, len(sorted_idx))):
    #                     q_idx = sorted_idx[i].item()
    #                     cost_val = sorted_costs[i].item()
    #                     if cost_val != float("inf"):
    #                         all_candidates.append((q_idx, gt_i + sum(gt_groups[:b]), cost_val))  # batch-level GT index
    #
    #             # Step 3: Resolve query conflicts (keep min-cost match)
    #             best_query_to_gt = {}
    #             for q, g, cost in all_candidates:
    #                 if q not in best_query_to_gt or best_query_to_gt[q][1] > cost:
    #                     best_query_to_gt[q] = (g, cost)
    #
    #             # Step 4: Final indices
    #             if best_query_to_gt:
    #                 pred_idx = torch.tensor(list(best_query_to_gt.keys()), dtype=torch.long, device=c.device)
    #                 gt_idx = torch.tensor([best_query_to_gt[q][0] for q in pred_idx.tolist()], dtype=torch.long,
    #                                       device=c.device)
    #                 topk_match_indices.append((pred_idx, gt_idx))
    #             else:
    #                 topk_match_indices.append((
    #                     torch.zeros(0, dtype=torch.long, device=c.device),
    #                     torch.zeros(0, dtype=torch.long, device=c.device)
    #                 ))
    #         return hungarian_match_indices, topk_match_indices
    #     else:
    #         return hungarian_match_indices, None
