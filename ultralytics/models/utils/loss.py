...
from ultralytics.utils.loss import FocalLoss, VarifocalLoss, VarifocalLossWithSmoothing, MatchAwareLoss, MatchAwareLossWithSmoothing
from ultralytics.utils.metrics import bbox_iou, adaptive_wasserstein_loss
...
class DETRLoss(nn.Module):
    ...
    def __init__(self, nc=80, imgsz=640, loss_gain=None, aux_loss=True, use_fl=True, use_vfl=False, use_mal=False,
                 label_smoothing=False, use_aal=False, use_topk=False, use_uni_match=False, uni_match_ind=0):
        """
        ...
        Args:
            ...
            use_fl (bool): Whether to use FocalLoss.
            use_vfl (bool): Whether to use VarifocalLoss.
            use_mal (bool): Whether to use Matchability-aware Loss.
            label_smoothing (bool): Enable selective label smoothing.
            topk_loss (bool): Whether to use Top-k Loss.
        ...
        if use_topk:
            self.topk = 5
            self.hungarian_weight = 0.7
            self.topk_weight = 0.3

        self.nc = nc
        self.imgsz = imgsz
        self.matcher = HungarianMatcher(cost_gain={"class": 2, "bbox": 5, "giou": 2}, topk=self.topk if use_topk else 0)
        self.loss_gain = loss_gain
        self.aux_loss = aux_loss
        self.label_smoothing = label_smoothing
        self.use_aal = use_aal
        ...
        if use_vfl:
            if label_smoothing:
                self.vfl = VarifocalLossWithSmoothing(smoothing=0.1)
            else:
        ...
        if use_mal:
            if label_smoothing:
                self.mal = MatchAwareLossWithSmoothing(smoothing=0.1)
            else:
                self.mal = MatchAwareLoss()
        else:
            self.mal = None
    ...
    def _get_loss_class(self, pred_scores, targets, gt_scores, num_gts, postfix=""):
        ...
        Notes:
            ...
            - Matchability-aware Loss (if self.mal is True and num_gts > 0)
    ...
    def _get_loss_bbox(self, pred_bboxes, gt_bboxes, postfix=""):
        ...
        if self.use_aal:
            # Adaptive wasserstein loss combined with giou loss
            ratio = 0.3
            bboxes_area = (gt_bboxes[:, 2] * gt_bboxes[:, 3]).clamp(min=1e-6)
            nwd = 1.0 - adaptive_wasserstein_loss(pred_bboxes * self.imgsz, gt_bboxes * self.imgsz, bboxes_area)
            giou = 1.0 - bbox_iou(pred_bboxes, gt_bboxes, xywh=True, GIoU=True)
            aal = ratio * giou + (1 - ratio) * nwd
            loss[name_giou] = self.loss_gain["giou"] * aal.sum() / len(gt_bboxes)
        else:
            giou = 1.0 - bbox_iou(pred_bboxes, gt_bboxes, xywh=True, GIoU=True)
            loss[name_giou] = self.loss_gain["giou"] * giou.sum() / len(gt_bboxes)
    ...
    # ):
    #     if match_indices is None:
    #         match_indices, topk_match_indices = self.matcher(
    #             pred_bboxes, pred_scores, gt_bboxes, gt_cls, gt_groups, masks=masks, gt_mask=gt_mask
    #         )
    #     else:
    #         topk_match_indices = None
    #
    #     # ===  Hungarian match ===
    #     (batch_idx, src_idx), dst_idx = self._get_index(match_indices)
    #     pred_bboxes_main = pred_bboxes[batch_idx, src_idx]
    #     gt_bboxes_main = gt_bboxes[dst_idx]
    #
    #     bs, nq = pred_scores.shape[:2]
    #     targets = torch.full((bs, nq), self.nc, device=pred_scores.device, dtype=gt_cls.dtype)
    ...
    #
    #     gt_scores = torch.zeros([bs, nq], device=pred_scores.device)
    #     if len(gt_bboxes_main):
    #         gt_scores[batch_idx, src_idx] = bbox_iou(pred_bboxes_main.detach(), gt_bboxes_main, xywh=True).squeeze(-1)
    #
    #     loss_hungarian = {
    #         **self._get_loss_class(pred_scores, targets, gt_scores, len(gt_bboxes_main), postfix),
    #         **self._get_loss_bbox(pred_bboxes_main, gt_bboxes_main, postfix),
    #     }
    #
    #     if self.use_topk:
    #         losses = {k: self.hungarian_weight * v for k, v in loss_hungarian.items()}
    #     else:
    #         losses = loss_hungarian
    #
    #     # ===  Top-k match ===
    #     if topk_match_indices is not None:
    #         (batch_idx_topk, src_idx_topk), dst_idx_topk = self._get_index(topk_match_indices)
    #
    #         if len(src_idx_topk):
    #             pred_bboxes_topk = pred_bboxes[batch_idx_topk, src_idx_topk]
    #             gt_bboxes_topk = gt_bboxes[dst_idx_topk]
    #
    #             targets_topk = torch.full((bs, nq), self.nc, device=pred_scores.device, dtype=gt_cls.dtype)
    #             targets_topk[batch_idx_topk, src_idx_topk] = gt_cls[dst_idx_topk]
    #
    #             gt_scores_topk = torch.zeros([bs, nq], device=pred_scores.device)
    #             gt_scores_topk[batch_idx_topk, src_idx_topk] = bbox_iou(
    #                 pred_bboxes_topk.detach(), gt_bboxes_topk, xywh=True
    #             ).squeeze(-1)
    #
    #             loss_topk = {
    #                 **self._get_loss_class(pred_scores, targets_topk, gt_scores_topk, len(gt_bboxes_topk), postfix),
    #                 **self._get_loss_bbox(pred_bboxes_topk, gt_bboxes_topk, postfix),
    #             }
    #
    #             for k in loss_topk:
    #                 losses[k] = losses.get(k, 0) + self.topk_weight * loss_topk[k]
    #
    #     return losses
