...
from ultralytics.data import YOLODataset, EventDataset
from ultralytics.data.augment import Compose, Format, Format_event, v8_transforms, v8_transforms_event
...
class RTDETRDataset_event(EventDataset):
    """
    Real-Time DEtection and TRacking (RT-DETR) dataset class extending the base YOLODataset class.
    ...
    def build_transforms(self, hyp=None):
        ...
        if self.augment:
            hyp.mosaic = hyp.mosaic if self.augment and not self.rect else 0.0
            hyp.mixup = hyp.mixup if self.augment and not self.rect else 0.0
            transforms = v8_transforms_event(self, self.imgsz, hyp, stretch=True)
        else:
            transforms = Compose([])
        transforms.append(
            Format_event(
